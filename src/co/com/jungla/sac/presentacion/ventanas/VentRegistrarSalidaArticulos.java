...
public class VentRegistrarSalidaArticulos extends JFrame {
	...
	private EntradaOSalidaArticulo codigoSalida;
	private DetalleEntradaOSalida ultimoDetalleSalida;
	EntradaOSalidaArticulo salidaArticulo = new EntradaOSalidaArticulo();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					VentRegistrarSalidaArticulos frame = new VentRegistrarSalidaArticulos();
	 ...
	 * Create the frame.
	 */
	public VentRegistrarSalidaArticulos() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(VentRegistrarSalidaArticulos.class.getResource("/co/com/jungla/sac/presentacion/imagenes/Logo Jungla.png")));
		setTitle("Salida de Articulos del Inventario");
		...
		JLabel lblCostoTotal = new JLabel("Costo PROM");
		...
		lblCostoTotal.setBounds(21, 0, 79, 14);
		...
		JButton btnRegistrarEntrada = new JButton("Registrar Salida");
	...
	private void registrarSalidaArticulo() {
		DelegadoEntradaOSalidaArticulo delegadoSalidaArticulo = new DelegadoEntradaOSalidaArticulo();
		
		salidaArticulo.setConcepto("Salida");
		salidaArticulo.setFecha(new Date());
		salidaArticulo.setCantTotalArticulos(Integer.parseInt(txtItems.getText()));
		salidaArticulo.setTotalCosto(Double.parseDouble(desformatearNumero(txtTotalCosto.getText())));
		salidaArticulo.setObservaciones(txpObservaciones.getText());
		
		delegadoSalidaArticulo.insertarEntradaOSalidaArticulo(salidaArticulo);
		
		codigoSalida = delegadoSalidaArticulo.traerUltimaEntradaOSalida();
		
		registrarDetalleSalida();
		abrirVentanaSalidaRegistrada();
		
	}
	//Metodo apra registrar el detalle de la entrada
	private void registrarDetalleSalida() {
		DelegadoDetalleEntradaOSalida delegadoDetalleSalida = new DelegadoDetalleEntradaOSalida();
		DetalleEntradaOSalida detalleSalida = new DetalleEntradaOSalida();
		Articulo articulo = new Articulo();
		LineaArticulos lineaArticulos = new LineaArticulos();
		
		for(int i=0; i< modeloTbArticulos.getRowCount();i++){
			
			salidaArticulo.setIdEntradaOSalida(codigoSalida.getIdEntradaOSalida());
			detalleSalida.setEntradaOSalidaArticulo(salidaArticulo);
			...
			detalleSalida.setArticulo(articulo);
			detalleSalida.setCantidad(Integer.parseInt(modeloTbArticulos.getValueAt(i, 3).toString()));
			detalleSalida.setCostoUnitario(Double.parseDouble(desformatearNumero(modeloTbArticulos.getValueAt(i, 4).toString())));
			detalleSalida.setTotal(Double.parseDouble(desformatearNumero(modeloTbArticulos.getValueAt(i, 5).toString())));
			
			delegadoDetalleSalida.insertarDetalleEntradaOSalida(detalleSalida);
			
			registrarDetalleSalidaAlKardex();
		}
	}
	//Metodo para registrar el detalle de la entrada al kardex electronico de articulos
	private void registrarDetalleSalidaAlKardex() {
		...
		ultimoDetalleSalida = delegadoDetalleSalida.traerUltimoDetalleEntradaOSalida();
		
		if(delegadoKardexElectronico.traerUltimoRegistroKardexElectronicopPorArticulo(ultimoDetalleSalida.getArticulo().getCodigo())==null){
			kardexElectronico.setFecha(ultimoDetalleSalida.getEntradaOSalidaArticulo().getFecha());
			kardexElectronico.setMovimiento(ultimoDetalleSalida.getEntradaOSalidaArticulo().getConcepto());
			kardexElectronico.setNroDocumento(ultimoDetalleSalida.getEntradaOSalidaArticulo().getIdEntradaOSalida());
			articulo.setCodigo(ultimoDetalleSalida.getArticulo().getCodigo());
			...
			kardexElectronico.setCantidadSalida(ultimoDetalleSalida.getCantidad());
			kardexElectronico.setCostoUSalida(ultimoDetalleSalida.getCostoUnitario());
			kardexElectronico.setCostoTSalida(ultimoDetalleSalida.getTotal());
			kardexElectronico.setCantidadSaldo(ultimoDetalleSalida.getCantidad());
			kardexElectronico.setCostoUSaldo(ultimoDetalleSalida.getCostoUnitario());
			kardexElectronico.setCostoTSaldo(ultimoDetalleSalida.getTotal());
			kardexElectronico.setProveedorUltimaCompra("");
			
			delegadoKardexElectronico.insertarKardexElectronico(kardexElectronico);
			
			controlInventario = delegadoControlInventario.traerContInventarioPorCodigoArticulo(ultimoDetalleSalida.getArticulo().getCodigo());
			controlInventarioAModificar = controlInventario.get(0);
			
			controlInventarioAModificar.setCostoPromedio(ultimoDetalleSalida.getCostoUnitario());
			controlInventarioAModificar.setCantExistencia(ultimoDetalleSalida.getCantidad());
			controlInventarioAModificar.setTotalCostoInventario(ultimoDetalleSalida.getTotal());
				
			delegadoControlInventario.actualizarControlInventario(controlInventarioAModificar);
		}else{
			
			ultimoRegistroPorArticuloKardex1 = delegadoKardexElectronico.traerUltimoRegistroKardexElectronicopPorArticulo(ultimoDetalleSalida.getArticulo().getCodigo());
			...
			kardexElectronico.setCostoUSalida(ultimoDetalleSalida.getCostoUnitario());
			kardexElectronico.setCostoTSalida(ultimoDetalleSalida.getTotal());
			kardexElectronico.setCantidadSaldo(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad());
			kardexElectronico.setCostoUSaldo((ultimoRegistroPorArticuloKardex.getCostoTSaldo() - (ultimoDetalleSalida.getCostoUnitario())*ultimoDetalleSalida.getCantidad())/(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad()));
			kardexElectronico.setCostoTSaldo((ultimoRegistroPorArticuloKardex.getCostoTSaldo() - ultimoDetalleSalida.getTotal()/(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad())*(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad())));
			kardexElectronico.setProveedorUltimaCompra("");
			...
			controlInventarioAModificar.setCostoPromedio((ultimoRegistroPorArticuloKardex.getCostoTSaldo() - (ultimoDetalleSalida.getCostoUnitario())*ultimoDetalleSalida.getCantidad())/(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad()));
			controlInventarioAModificar.setCantExistencia(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad());
			controlInventarioAModificar.setTotalCostoInventario((ultimoRegistroPorArticuloKardex.getCostoTSaldo() - ultimoDetalleSalida.getTotal()/(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad())*(ultimoRegistroPorArticuloKardex.getCantidadSaldo() - ultimoDetalleSalida.getCantidad())));
	...
	private void abrirDialogoConfirmacionRegistro() {
		...
		int res = JOptionPane.showConfirmDialog(null, "Esta seguro de grabar esta SALIDA de ARTICULOS del Inventario?"+nl+nl+"Costo Total: "+txtTotalCosto.getText()+nl+"Items: "+txtItems.getText(), null, JOptionPane.YES_NO_OPTION);
		if(res==0){
	...
	private void abrirVentanaSalidaRegistrada() {
		VentMostrarEntradaOSalidaRegistrada ventMostrarSalidaRegistrada= new VentMostrarEntradaOSalidaRegistrada(txtItems.getText(), txtTotalCosto.getText(), txpObservaciones.getText(), "Salida");
		ventMostrarSalidaRegistrada.setVisible(true);
		limpiarDatos();
